...
| 跨 runtime 投影验证 | 部分完成 | Claude / Codex / OpenClaw 有 live pass；Cursor smoke pass 但 live harness unavailable；runtime evidence packet 统一记录 `status`、`evidenceKind`、`failureClass`、`command`、`artifact`、`remainingAction` | 还缺 Cursor native live-turn harness，以及 Claude / OpenClaw 全 meta agents shard evidence |
...
| T-002 | Claude / Codex / Cursor / OpenClaw 四端投影验证 | 部分完成 | Claude / OpenClaw live pass；Codex live pass；Cursor smoke pass + live harness unavailable | 四端都有 live、smoke 或 unsupported-with-reason 证据；发布级完成还需要 Cursor live harness 与全 meta agents shard evidence |
...
1. 并行推进 P-004 / P-005，让 Cursor 有 native live 证据或正式 blocked 边界。
2. 同时跑 P-006 / P-007，把 Claude / OpenClaw 从单 agent live 扩成 shard evidence。
