...
- 本地已有但 `origin/main` 仍未包含：Cursor live harness blocked contract、runtime evidence aggregator / writeback approval、P-015 到 P-021 编排质量轨、以及本 PRD v0.16 的状态更新。
- 仍未完成且不能对 GitHub 宣称完成：P-006 / P-007 全 meta agents shard evidence、P-012 / P-013 / P-014 / P-022 / P-023 用户交付轨、P-024 Cursor native live pass 解阻。
...
| T-002 | Claude / Codex / Cursor / OpenClaw 四端投影验证 | 部分完成 | Claude / OpenClaw live pass；Codex live pass；Cursor smoke pass + native live blocked-with-contract | 四端都有 live、smoke 或 unsupported-with-reason 证据；发布级完成还需要 Cursor native live pass 与全 meta agents shard evidence |
...
| P-022 | R-010 | 产品面板数据合同：先定义 Web/UI 面板读取的 runId JSON schema，再做页面，避免 UI 直接解析内部协议碎片 | 未完成 | backend | 可 | `runReportPanelContract` 含 decision summary、owner、blocked reason、runtime evidence、approval request、course rubric；测试能从 latest run 生成该合同 |
| P-023 | R-011 | AI 课案例包：把一次 run 的设计、执行、验收、反馈、交付内容做成课程 reviewer 能直接看懂的示例 | 未完成 | docs | 可 | 生成 Markdown 示例，包含“学员该看到什么”“老师怎么评分”“哪些证据算通过/失败” |
| P-024 | R-008 | Cursor native live pass 解阻：在有可用 `cursor-agent` 或支持 `-p --output-format json` 的 `cursor agent` 后，把 P-005 从 blocked 边界升级为真实 live pass | 阻塞 | verify | 可 | `node scripts/eval-meta-agents.mjs --runtime=cursor --live` exit 0，`cursor.status = passed`，`evidenceKind = live`，`releaseGrade` 仍需四端综合判断 |

当前主干闭合顺序：

1. 已完成 P-004 / P-005 的合同和 blocked 边界：Cursor projection smoke 仍可用，但本机缺可解析 native live harness 时不会冒充 live pass。
2. Runtime 证据轨并行跑 P-006 / P-007；P-024 先保持阻塞，等环境具备 Cursor Agent CLI 后再升级为 live pass。
...
6. 下一步并行跑用户交付轨 P-012 / P-013 / P-014 / P-022 / P-023，先用数据合同和课程评分表约束 UI 与报告，而不是先做漂亮页面。

当前可并行批次：

| 批次 | 可并行任务 | 合并 owner | 依赖关系 | 不可并行点 |
|---|---|---|---|---|
| Runtime evidence | P-006、P-007、P-024 | meta-conductor | P-006 / P-007 可立即开子窗口；P-024 依赖 Cursor Agent CLI 环境 | P-024 没有可用 CLI 前只能保留 blocked，不可伪造 pass |
...
| User deliverable | P-012、P-013、P-014、P-022、P-023 | meta-warden | P-022 数据合同先约束 P-012；P-013/P-014/P-023 可独立产出 review/rubric/example | 页面不得先于数据合同宣称完成 |
| Release closure | P-006..P-024 全部证据回收后 | meta-warden | 需要 smoke、targeted tests、子窗口证据、PRD 状态更新、GitHub 差距报告 | 任何 blocked 项必须留 remainingAction，不能算发布级完成 |
...
| Cursor | smoke pass；native live blocked-with-contract | `node scripts/eval-meta-agents.mjs --runtime=cursor` pass；`node scripts/eval-meta-agents.mjs --runtime=cursor --live` 返回 `status = "blocked"`、`failureClass = "native_harness_missing"`、合同 `cursor-live-turn-harness-v0.1`；本机 `cursor` 3.4.13 只证明 IDE launcher / subcommand help，未证明 `-p --output-format json` Agent 输出 | 安装/暴露可解析的 Cursor Agent CLI（`cursor-agent` 或真正支持 print/json 的 `cursor agent`），再把 blocked 升级为 native live pass |
...
- 官方 evidence：Cursor CLI overview / using / parameters 文档声明 Agent CLI、non-interactive print mode、`--output-format json`。
- 本机 evidence：`cursor --version` 为 3.4.13；`where cursor-agent` 未找到；`cursor agent --help` 没有 `--print` / `--output-format`，因此 `node scripts/eval-meta-agents.mjs --runtime=cursor --live` 必须返回 structured blocked。
