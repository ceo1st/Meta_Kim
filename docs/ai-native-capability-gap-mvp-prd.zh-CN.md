...
| 完整产品验收 | 已测通 | `npm run meta:release:smoke`、`tests/meta-theory/30-capability-gap-complete-product.test.mjs`、`tests/meta-theory/32-meta-theory-four-product-targets.test.mjs` | 后续扩展 live runtime 验证 |
...
| 跨 runtime 投影验证 | 已测通 | `meta:theory:run` 记录 Claude / Codex / Cursor / OpenClaw projection smoke evidence 和 runtime 差异 | OpenClaw / Cursor 仍只标 smoke，不声明 typed plugin/native choice live pass |
...
| T-002 | Claude / Codex / Cursor / OpenClaw 四端投影验证 | 已测通 | `meta:theory:run` 生成四端 projection smoke evidence；Codex 主窗口和子窗口复测 | 四端都有 live 或 smoke 证据，能自然走新 orchestration 路线，并记录 runtime 差异 |
