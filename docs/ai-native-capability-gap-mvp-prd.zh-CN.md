...
| 完整产品验收 | 已测通 | `npm run meta:release:smoke`、`tests/meta-theory/30-capability-gap-complete-product.test.mjs`、`tests/meta-theory/32-meta-theory-four-product-targets.test.mjs` | 后续闭合 Cursor live harness 和全 meta agents shard evidence |
...
| 跨 runtime 投影验证 | 部分完成 | Claude / Codex / OpenClaw 有 live pass；Cursor smoke pass 但 live harness unavailable | 还缺 Cursor native live-turn harness，以及 Claude / OpenClaw 全 meta agents shard evidence |
| Warden 审批写回 | 已测通 | `tests/meta-theory/32-meta-theory-four-product-targets.test.mjs` 用临时 canonicalRoot 证明 `candidate_only` 与 `approved-for-writeback` 受控写入 | 后续在真实 Warden 人工批准后才允许写当前 repo canonical |
| 用户可读产品面板 | 已测通 | `npm run meta:theory:report -- --run-id latest` 可按 runId 读取中文报告 | 后续可升级为 Web/UI 面板 |
| AI 课可理解产品标准 | 已测通 | `config/contracts/ai-course-product-standards.json`、完整产品报告中的“AI 课可理解标准” | 后续把标准用于课程示例和人工评分表 |

当前可以宣称的结论：

- Complete product MVP 已经在本地证明：设计、执行、验收、反馈、交付内容都有可复查证据，且能按 AI 课可理解标准解释。
- 还不能宣称“发布级 live/native 全 runtime 完成”，因为 Cursor 还没有 native live-turn harness，Claude / OpenClaw 还只完成 `meta-warden` live shard；真实外部写回仍需要 Warden 明确批准。
...
| T-002 | Claude / Codex / Cursor / OpenClaw 四端投影验证 | 部分完成 | Claude / Codex / OpenClaw live pass；Cursor smoke pass + live harness unavailable | 四端都有 live、smoke 或 unsupported-with-reason 证据；发布级完成还需要 Cursor live harness 与全 meta agents shard evidence |
...
| T-004 | 用户可读 UI / 报告层 | 已测通 | `npm run meta:theory:report -- --run-id latest`、runId 中文 Markdown 报告 | 用户可按 runId 查看判定原因、阻塞原因、下一步 owner、升级长期能力建议和验证状态 |
| T-005 | AI 课可理解产品标准 | 已测通 | `config/contracts/ai-course-product-standards.json`、`tests/meta-theory/30-capability-gap-complete-product.test.mjs` | 设计、执行、验收、反馈、交付内容都有人话问题、通过标准、失败标准和证据字段 |
...
| P-002 | R-008 | Codex session recovery：若 live 仍超时，记录可恢复 runId、stderr、timeout stage 和下一次续跑策略 | 部分完成 | backend | 可 | timeout fallback 已实现为 `codex_live_timeout`、`stage`、`retryCommand`、stdout/stderr tail；还缺强制 timeout fixture 或子窗口复核 |
| P-003 | R-008 | Codex 主窗口 / 子窗口隔离复测：子窗口只跑测试与证据采集，主窗口只合并和审查 | 进行中 | test | 可 | 已创建子窗口 `019e9163-31ec-7510-86f9-9fc645c95811`；等待其返回 threadId、命令、artifact、主窗口 review 结论 |
| P-004 | R-008 | Cursor native live-turn harness 设计：定义 Cursor 原生触发、输入、输出、可观测证据和 unsupported 条件 | 未完成 | docs | 可 | 新 contract 或 PRD 小节能说明 Cursor smoke 与 live 的边界 |
| P-005 | R-008 | Cursor native live-turn harness 实现或明确阻塞：从 projection smoke 升级到 native live，或写清宿主限制 | 未完成 | backend | 可 | `--runtime=cursor --live` 返回 live pass，或返回结构化 blocked/unsupported-with-reason |
| P-006 | R-008 | Claude 全 meta agents shard：从 `meta-warden` 扩到九个 meta agents，必要时分片执行 | 未完成 | verify | 可 | Claude live 报告含 shard plan、每个 agent status、失败回退 |
| P-007 | R-008 | OpenClaw 全 meta agents shard：从 `meta-warden` 扩到九个 meta agents，验证 hooks / config / model 稳定性 | 未完成 | verify | 可 | OpenClaw live 报告含 MiniMax M3、hooks ready、每个 shard status |
| P-008 | R-008 | 四端 evidence aggregator：把 live/smoke/skipped/blocked 统一写入 RunStateStore 和中文报告 | 未完成 | backend | 可 | 报告能按 runtime 查询 status、command、artifact、remaining action |
| P-009 | R-008 | runtime failure taxonomy：把 timeout、auth missing、native harness missing、projection-only、tool unsupported 分成固定失败类 | 未完成 | meta-prism | 可 | 测试断言失败类型不被误写成 pass |
| P-010 | R-009 | 真实 Warden approval packet：定义当前 repo canonical 写回前必须出现的人工批准证据格式 | 未完成 | meta-warden | 可 | approval packet 含 approver、scope、target、diff summary、rollback plan |
| P-011 | R-009 | 当前 repo canonical writeback dry-run：不写 canonical，只生成将写入内容、目标路径和风险审查 | 未完成 | meta-chrysalis | 可 | dry-run artifact 证明写入数量为 0，且候选可审查 |
| P-012 | R-010 | Web/UI 产品面板原型：按 runId 展示 GapDecision、owner、阻塞原因、升级建议和 runtime evidence | 未完成 | frontend | 可 | 本地页面或静态 artifact 可打开，且不泄露本机绝对路径 |
| P-013 | R-010 | 报告可读性 review：把协议字段翻译成人话标签，保留机器字段但不让用户被字段淹没 | 未完成 | review | 可 | 生成前后报告对照，AI 课 reviewer 能看懂每个结论 |
| P-014 | R-011 | AI 课评分表导出：把五维标准变成课程 reviewer 可直接打分的 Markdown / JSON rubric | 未完成 | docs | 可 | rubric 覆盖设计、执行、验收、反馈、交付内容，并映射 artifact evidence |
| P-015 | R-001/R-011 | 多 gap 混合需求 fixture：同类型多需求、多类型并发、共享 owner、线性依赖都要覆盖 | 未完成 | test | 可 | 新 fixture 证明分组但不折叠 worker 实例 |
| P-016 | R-007/R-008 | orchestration board 并行/线性计划质量门：验证 dependsOn、parallelGroup、mergeOwner、reviewOwner 不冲突 | 未完成 | meta-conductor | 可 | validator 或测试能指出空依赖、错 merge owner、循环依赖 |

当前主干闭合顺序：

1. 先完成 P-002 / P-003 的复核闭环，确保 Codex live pass 和 timeout fallback 都有独立证据。
2. 并行推进 P-004 / P-005，让 Cursor 有 native live 证据或正式 blocked 边界。
3. 同时跑 P-006 / P-007，把 Claude / OpenClaw 从单 agent live 扩成 shard evidence。
4. P-008 / P-009 负责把四端证据变成产品可读报告，防止 pass、smoke、skipped、blocked 混写。
5. P-010 / P-011 等 Warden 明确批准后才进入真实 canonical writeback；批准前只能 dry-run。
...
|---|---|---|---|
| Claude | live pass | `node scripts/eval-meta-agents.mjs --runtime=claude --live --agent=meta-warden`，`summary.passed = ["claude"]` | 扩展到全 meta agents 或记录 shard plan |
...
| OpenClaw | live pass | `node scripts/eval-meta-agents.mjs --runtime=openclaw --live --agent=meta-warden`，MiniMax M3，hooks ready，`summary.passed = ["openclaw"]` | 扩展到全 meta agents 或记录 shard plan |
| Cursor | smoke pass，live unsupported-with-reason | `node scripts/eval-meta-agents.mjs --runtime=cursor` pass；`node scripts/eval-meta-agents.mjs --runtime=cursor --live --require-all-runtimes` 返回 `cursor_live_harness_unavailable` | 实现 Cursor native live-turn harness，不能用 projection smoke 冒充 native/live pass |
