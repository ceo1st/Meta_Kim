...
Capability invocation truth: every governed run that names agents/subagents, app-visible host UI subagents, skills, MCP, hooks, prompts/rules, commands/scripts, runtime tools, memory, graph, `agent-teams-playbook`, or worker tasks must classify each capability family as `invoked`, `selected_not_invoked`, `discovered_not_selected`, `unavailable`, `blocked`, or `not_required`. A selected skill, selected `agent-teams-playbook` provider, configured MCP server, matched hook, prompt update, command candidate, runtime tool candidate, host UI subagent badge, or run-scoped worker is not an invoked runner capability without attached invocation evidence. If no real runtime Agent/subagent tool call is available or attached, say so and keep peer workers as run-scoped structural workers; do not call them live subagents or peer-to-peer runtime agents. If the host UI visibly shows subagents, record that separately as `app_visible_subagent` evidence and do not relabel it as Meta_Kim runner `spawn_agent` / `Agent` invocation unless the tool-call evidence is attached. If `agent-teams-playbook` is selected, record it separately as `agent_teams_playbook=selected_not_invoked` unless a live Skill/Agent Team/spawn_agent call is attached.
