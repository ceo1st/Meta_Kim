...
Fan-out eligibility is only the complexity decision; live host-agent dispatch also requires runtime authorization and a callable host surface. When fan-out eligible, Thinking must produce `workerTaskPackets` before Execution. In `fan_out_ready` state, if the active host exposes an authorized Task/Agent/`spawn_agent` surface, the main thread MUST dispatch all independent `workerTaskPackets` that share a `parallelGroup` as concurrent tool calls in a single assistant message — one tool call per packet. Serial dispatch across multiple messages in authorized `fan_out_ready` state is a gate violation (fake parallelism), not a fallback. For Codex, direct subagent/delegation/parallel-agent wording or a completed native choice surface is required before live subagent fan-out; a `meta-theory` trigger alone enters governed routing but does not prove Codex subagent authorization. Direct user corrections such as "我要的是派发 / 并行 / 多 agent" are governed execution entries and direct fan-out authorization; route them through owner-first capability discovery instead of treating them as low-signal chat. If the host tool is unavailable, rejected, or unauthorized, record `subagentCapabilityStatus`, `degradationReason`, and `hostInvocationRequestPacket`; do not silently serialize without a reason or claim live fan-out.
