...
1. Thinking 阶段写入 **`intentPacket`**（`trueUserIntent`、`successCriteria`、`nonGoals`、`intentPacketVersion: v1`）——重执行前的意图锁定（见 `contracts/workflow-contract.json` 中 `protocols.intentPacket` 与 `runDiscipline.protocolFirst.intentPacketRequiredWhenGovernanceFlows`）。
2. 按 `runDiscipline.protocolFirst.requiredPackets` 维护完整 packet 链（可每轮增量合并）。
3. 在宣称 **对外可发** 或 **已完成** 之前执行：

```bash
npm run validate:run -- path/to/your-run.json
```

4. 若校验失败或 finding 仍开：生成下一轮待办清单：
